...
class CreateOrdersTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('orders', function (Blueprint $table) {
            ...
            $table->dateTime('paid_at')->nullable()->comment('支付时间');//bnullable 可以为空
            $table->string('payment_method')->nullable()->comment('支付方式');
            $table->string('payment_no')->nullable()->comment('支付平台订单号');
            $table->string('refund_status')->default(\App\Models\Order::REFUND_STATUS_PENDING)->comment('退款状态');
            $table->string('refund_no')->nullable()->comment('退款状态');
            $table->boolean('closed')->default(false)->comment('订单是否已经关闭');
            $table->boolean('reviewed')->default(false)->comment('是否评论');
